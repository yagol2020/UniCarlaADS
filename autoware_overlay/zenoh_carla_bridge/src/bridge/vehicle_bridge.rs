...
pub struct VehicleBridge<'a> {
    ...
    // UniCarlaADS：bridge 只计算控制量，不再直接写入 CARLA，交由宿主 apply_control。
    last_carla_control: VehicleControl,
}
...
impl<'a> VehicleBridge<'a> {
    ...
    fn pub_current_actuation(&mut self, timestamp: f64) -> Result<()> {
        // UniCarlaADS：发布当前指令控制量，而不是 CARLA actor 的实际控制量。
        let control = &self.last_carla_control;
    ...
    fn update_carla_control(&mut self, timestamp: f64) {
        ...
        self.last_carla_control = VehicleControl {
        ...
        log::debug!(
            "Bridge => Control (host applies): throttle={:.3}, steer={:.3}, brake={:.3}, hand_brake={}, reverse={}",
            accel_cmd as f32,
            steer,
            brake_cmd as f32,
            hand_brake,
